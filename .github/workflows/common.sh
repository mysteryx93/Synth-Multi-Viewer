...
desktop_csproj() {
  ...
  printf '%s\n' "$repo_root/SynthMultiViewer/SynthMultiViewer.csproj"
}
...
deploy_dir() {
  ...
  printf '%s\n' "$repo_root/SynthMultiViewer/Deploy"
}
...
read_project_value() {
  ...
  value=$(xml_value "$key" "$repo_root/Directory.Build.props")
...
output_dir() {
  ...
  printf '%s\n' "${PUBLISH_OUT:-$repo_root/artifacts/$version}"
}
